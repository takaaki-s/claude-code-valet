...
func (m HelpModel) View() string {
	...
	// does. This popup is where a user goes to ask why.
	//
	// The second line names the cursor rather than promising a second tap: what a
	// tap does is move the CURSOR, so a tap on the row it already sits on attaches
	// straight away — and since the cursor starts on the first session, that row's
	// behaviour is the first thing a new user meets. The rule in full is written
	// down where the behaviour is, in handleMouse.
	//
	// Both lines stay inside the longest description already in this view. The
	// popup is sized as a percentage of the terminal, so a wider line wraps on the
	// small terminals the two-tap click was added for — and this view has no
	// scrolling, so the one line this pair added raised the terminal height the
	// popup needs from 47 rows to 49.
