...
// "<plugin> / default" — the action ID stays visible so the help view and the
// config file speak the same vocabulary. Actions with no keys are omitted; only
// the first key is shown (same convention as the palette).
